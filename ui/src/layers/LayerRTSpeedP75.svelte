<script lang="ts">
    ...
    function formatSpeedLabel(wayId: string): string {
        const speed = geoData.wayData[wayId]?.speed_p75;
    ...
    function getSpeedStyle(wayId: string): L.PathOptions {
        ...
        const speed_p75 = props?.speed_p75;
        ...
        if (
            speed_p75 !== undefined &&
            speed_p75 !== null &&
            !isNaN(Number(speed_p75))
        ) {
            const speedValue = Number(speed_p75);
    ...
    $effect(() => {
        ...
        const filteredFeatures = geoData.features.filter(
            (feature: Feature | undefined) => {
                ...
                return (
                    props?.speed_p75 !== undefined &&
                    props?.speed_p75 !== null &&
                    !isNaN(Number(props?.speed_p75))
                );
            },
        );
        ...
        const newLayer = L.geoJSON(
            // Order by speed_p75 asc, to plot higher speeds on top
            filteredFeatures.sort((a, b) => {
                ...
                const propsB = b.properties?.way_osm_id
                    ...
                    : null;
                return (propsA?.speed_p75 || 0) - (propsB?.speed_p75 || 0);
            }),
            {
